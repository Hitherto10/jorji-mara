...
// From all variants of a product, extract unique option dimensions.
// e.g. sizes: ['xs','s','m','l','xl'], colors: [{name,hex}], pieces: ['Top','Skirt','Full Set']
function extractOptions(variants) {
    ...
    // Custom option (e.g. "Piece": Top/Skirt/Full Set)
...
// Given current selections, find the matching variant
...
// From product images, get the relevant ones for the current variant
function getImagesForVariant(allImages, variantId, productId) {
    // Images linked to this specific variant
    const variantImages = allImages
        .filter(img => img.variant_id === variantId)
        .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));

    // Shared images (no variant_id)
...
export default function ProductPage() {
    ...
    useEffect(() => {
        async function load() {
            ...
            // 4. Pre-select variant from URL ?variant=id
    ...
    // { fulfillment: { policy: { body }, shipping: { body, "estimated time" } }, compositionAndMaterials: [...] }
            ...
            <main className="font-[Bricolage_Grotesque] mt-20 max-w-7xl  mx-auto px-4 md:px-8 lg:px-12 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] xl:grid-cols-[1fr_480px] gap-8 lg:gap-12">
                    ...
                    <div className="flex flex-col gap-5">
                                    ...
                                    {sizes.map(size => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`min-w-[44px] px-3 h-9 text-xs uppercase tracking-wide border transition-all ${
                                                    selected
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {colors.map(c => {
                                        ...
                                        // Multi-hex colour (e.g. Black & White)
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {/* White border outline for white swatches */}
                        ...
                        {/* Size chart link — placeholder: shown if product has size_chart_id */}
                        ...
                        {isOutOfStock ? (
                            ...
                            <div className="flex flex-col gap-2 w-full max-w-75">
                                <div className={`flex items-center gap-2 text-xs font-medium py-1.5 w-fit uppercase tracking-wider ${
                                    isVeryLowStock 
                                        ? 'text-red-700 ' 
                                        : 'text-amber-700 '
                                }`}>
                            ...
                            <div className="flex items-center border border-stone-300 h-12">
                                <button
                                    ...
                                    className={`w-10 h-full flex items-center justify-center transition-colors ${
                                        quantity <= 1 ? 'bg-stone-50 text-stone-300 cursor-not-allowed' : 'hover:bg-stone-50'
                                    }`}
                                    disabled={quantity <= 1}
                                >
                                ...
                                <button
                                    ...
                                    className={`w-10 h-full flex items-center justify-center transition-colors ${
                                        isOutOfStock || quantity >= stock ? 'bg-stone-50 text-stone-300 cursor-not-allowed' : 'hover:bg-stone-50'
                                    }`}
